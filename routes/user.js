...
router.get('/', async function (req, res, next) {
    ...
    if (!req.session.loggedIn) {
        ...
        if (req.session.loggedIn) {
            ...
            walletBalance = (await usrhelper.getValetAmount(req.session.user._id)).toFixed(2)
...
router.get('/view-product/:id', verifyLogin, async (req, res) => {
    await prhelper.getProductDetails(req.params.id).then(async (product) => {
        let walletBalance=0
        let wallet = (await usrhelper.getValetAmount(req.session.user._id)).toFixed(2)
            if(wallet){
                walletBalance=wallet
            }
            let cartCount = await usrhelper.getCartCount(req.session.user._id)
            let category = await prhelper.getAllCategory()
            let productPrice = (parseFloat(product.price) - (parseFloat(product.price) * (parseFloat(product.offer) / 100))).toFixed(2)
            let actualPrice = productPrice
            for (i = 0; i < category.length; i++) {
                //console.log(category[i]['category-name']);
                if (category[i]['category-name'] === product.category) {
                    actualPrice = (productPrice - (productPrice * (parseFloat(category[i].offer) / 100))).toFixed(2)
                }
            }
...
router.post('/proceed-to-payment-address', verifyLogin, async (req, res) => {
    ...
    let category = await prhelper.getAllCategory()
    let total = await usrhelper.findCartTotal(user_id)
    let walletBalance = (await usrhelper.getValetAmount(req.session.user._id)).toFixed(2)
    req.session.amountPay = amountPay
    let paymentByWallet = false
    if (amountPay === 0) {
        await usrhelper.updateWallet(wallet - htotal, req.session.user._id)
    ...
    await prhelper.checkCoupon(userCoupon).then(async (data) => {
        ...
        if (data == false) {
            ...
            if (nowDate > data.end_date && nowDate < start_date) {
                ...
                await prhelper.couponUsed(data._id, req.session.user._id, data.offer).then(async () => {
                    let total = await usrhelper.findCartTotal(req.session.user._id)
                    res.json({
                        couponSuccess: true,
                        couponTotal: total
...
router.post('/cancel-order/:id', verifyLogin, async (req, res) => {
    await prhelper.cancelOrder(req.params.id).then(async (data) => {
        console.log("\nOrder Deletion details : " + JSON.stringify(data));
        let waletAmount = (await usrhelper.getValetAmount(req.session.user._id)).toFixed(2)
...
router.post('/save-textfeedback', verifyLogin, async (req, res) => {
    ...
    await prhelper.addFeedback(req.session.user._id, req.body.feedback).then(() => {
        res.json({
...
router.post('/userMessage', verifyLogin, async (req, res) => {
    await usrhelper.addMessage(req.body, req.session.user._id).then((data) => {
            res.json(true)
        })
        .catch((err) => {
            res.json(false)
