...
function generatePdf(Data,template){
  ...
  var html = fs.readFileSync("./views/admin/"+template, "utf8");
    var options = {
        ...
        header: {
            height: "45mm",
            contents: '<div style="text-align: center;"><h1>Author: Organic Store</h1></div>'
        },
        footer: {
            height: "28mm",
            contents: {
...
function generateLimidPdf(orders){
  var html = fs.readFileSync("./views/admin/template.html", "utf8");
    var options = {
        ...
        header: {
            height: "45mm",
            contents: '<div style="text-align: center;"><h3>Author: Organic Store</h3></div>'
        },
        footer: {
            height: "28mm",
            contents: {
...
for(i=0;i<order1.length;i++){
  ...
  orderXl[i]={
    ...
    Address:order1[i].address.address_collection.first_name+', '+order1[i].address.address_collection.second_name+', '+order1[i].address.address_collection.house_name+', '+order1[i].address.address_collection.street+', '+order1[i].address.address_collection.town+', '+order1[i].address.address_collection.pin,
...
xlname=convertJsonToExcel(orderXl)
  ...
  res.render('admin/view-reports',{path:filename1,admin:req.session.admin,xlpath:xlname})
})
...
router.get('/yearPdf',verifyAdmin,async (req,res)=>{
  ...
  res.render('admin/reports',{title:'Admin',admin:req.session.admin,pdf:true,path:filename})
  }
  else{
    res.redirect('/admin')
  }
...
router.get('/yearXml',verifyAdmin,async (req,res)=>{
  ...
  if(yearOrder){
    ...
    console.log('rtrtrtrtrtrtrtrtrtrtr : '+filename);
  res.render('admin/reports',{title:'Admin',admin:req.session.admin,xml:true,xlpath:filename})
  }
  else{
    res.redirect('/admin')
  }
