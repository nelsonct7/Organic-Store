...
module.exports={
            ...
            let products=await db.get().collection(collection.PRODUCT_COLLECTION).aggregate([
                ...
                },
                {
                    $project:{
                        ...
                        img_path:1,
                        categoryInfo:1,
                        productprice:{$subtract:['$price',{$multiply:['$price',{$divide:[{$toInt:'$offer'},100]}]}]}
                    }
                },
                {
                    $project:{
                        ...
                        categoryInfo:1,
                        productprice:1,
                        actualprice:{$round:[{$subtract:['$productprice',{$multiply:['$productprice',{$divide:[{$toInt:'$categoryInfo.offer'},100]}]}]},2]}
                    }

                }
            ]).toArray()
           // console.log('\nProduct Data : '+JSON.stringify(products[0]));
            resolve(products)
            // .find({deleted:false,status:'Available'}).toArray()
            // if(products){
            //     resolve(products)
            // }else{
            //     reject()
    ...
    getProductsByCategory:(catName)=>{
        ...
        return new Promise(async (resolve,reject)=>{
            let products=await db.get().collection(collection.PRODUCT_COLLECTION).aggregate([
                ...
                },
                {
                    $project:{
                        ...
                        actualPrice:{$subtract:['$price',{$multiply:['$price',{$divide:[{$toInt:'$offer'},100]}]}]}
                    }
                },
                {
                    $project:{
                        ...
                        img_path:1,
                        actualPrice:1,
                        finalPrice:{$subtract:['$actualPrice',{$multiply:['$actualPrice',{$divide:[{$toInt:'$categoryInfo.offer'},100]}]}]}
                    }
                },
                {
                    $project:{  
                        ...
                        'categoryInfo.offer':1,
                        finalPrice:1,
                        discount:{$round:[{$subtract:['$price','$finalPrice']},2]}
            ...
            //console.log('\n\nCategory products : '+JSON.stringify(products[1]));
            resolve(products)
        })
    },
    ...
    getProductInfoOffer:()=>{
        return new Promise(async (resolve,reject)=>{
            ...
            //console.log('\nProducts offer details : '+JSON.stringify(data));
            resolve(data)
        })
    },
    ...
    addoffer:(data)=>{
        return new Promise(async (resolve,reject)=>{
            let prId=data.prId
            await db.get().collection(collection.PRODUCT_COLLECTION).updateOne({_id:objectId(prId)},{
                $set:{
                    offer:data.offer,
    ...
    getCategoryByID:(catId)=>{
        return new Promise(async (resolve,reject)=>{
            await db.get().collection(collection.CATEGORY_COLLECTION).findOne({_id:objectId(catId)}).then((data)=>{
    ...
    addCatOffer:(data)=>{
    return new Promise((resolve,reject)=>{
        db.get().collection(collection.CATEGORY_COLLECTION).updateOne({_id:objectId(data.catId)},{
                $set:{
                    offer:data.offer,
                    offer_start:new Date().toString(),
                    offer_end:data.enddate,
    ...
    removeCatOffer:(prId)=>{
        return new Promise(async (resolve,reject)=>{
            await db.get().collection(collection.CATEGORY_COLLECTION).updateOne({_id:objectId(prId)},{
                $set:{
                    offer:0,
                    offer_start:null,
                    offer_end:null,
            ...
            console.log(data);
                ...
                if(data[0].status!='pending'){
                    ...
                    if(nowDate>lastDate){
                        ...
                        let wallet=await db.get().collection(collection.WALLET_COLLECTION).findOne({user_data:objectId(data1.user_data)})
                        console.log('TYTYRTYRTRYRRY : '+data1.user_data);
                        console.log(wallet);
                        if(wallet){
                            await db.get().collection(collection.WALLET_COLLECTION).updateOne({user_data:objectId(data1.user_data)},{
                                $inc:{amount:data1.total_amount}
                            })
                            for(i=0;i<data1.products.length;i++){
                                console.log(data1.products[i].quantity); 
                                await db.get().collection(collection.PRODUCT_COLLECTION).updateOne({_id:objectId(data1.products[i].product_id)},{
                            ...
                            await db.get().collection(collection.WALLET_COLLECTION).insertOne({user_data:data1.user_data,amount:data1.total_amount})
                            for(i=0;i<data1.products.length;i++){
                                await db.get().collection(collection.PRODUCT_COLLECTION).updateOne({_id:objectId(data1.products[i].product_id)},{
                                    $inc:{'$godown-stock':data1.products[i].quantity}
                                })
                            }
                            await db.get().collection(collection.ORDER_COLLECTION).remove({_id:objectId(ordId)})
                            resolve({orderDeleted:true})
                        }
                    }
    ...
    getTotalDashbord:()=>{
        ...
        //    console.log("\nDeemo : "+JSON.stringify(deemo));
           for(i=0;i<deemo.length;i++){
            ...
            dashbordObject.orderUnderProcessing=(dashbordObject.orderUnderProcessing-dashbordObject.deletedOrder)-dashbordObject.pendingOrder
    ...
    getCoupons:()=>{
        return new Promise( async(resolve,reject)=>{
            let data=await db.get().collection(collection.COUPON_COLLECTION).find().toArray()
            resolve(data)
        })
    ...
    removeCoupon:(coupId)=>{
        return new Promise(async (resolve,reject)=>{
            await db.get().collection(collection.COUPON_COLLECTION).remove({_id:objectId(coupId)}).then(()=>{
                resolve()
            })
        })
    },
    ...
    checkCoupon:(coupon)=>{
        return new Promise(async (resolve,reject)=>{
        let data=await db.get().collection(collection.COUPON_COLLECTION).aggregate([
                {
    ...
    couponUsed:(couponId,userId,offer)=>{
        return new Promise(async (resolve,reject)=>{
            await db.get().collection(collection.COUPON_COLLECTION).updateOne({_id:objectId(couponId)},{
                ...
                await db.get().collection(collection.CART_COLLECTION).updateOne({user_data:objectId(userId)},{
                    $set:{
            ...
            for(i=0;i<category.length;i++){
                if(category[i].offer!=0){
                    ...
                    if(nowDate>category[i].offer_end){
                        await db.get().collection(collection.CATEGORY_COLLECTION).updateOne({_id:objectId(category[i]._id)},{
                            $set:{
                                offer:0,
                                offer_end:null,
                                offer_start:null
    ...
    addFeedback:(userId,feedback)=>{
        ...
        return new Promise(async (resolve,reject)=>{
            let userPresent=await db.get().collection(collection.FEEDBACK_COLLECTION).findOne({user_data:objectId(userId)})
            if(userPresent){
                await db.get().collection(collection.FEEDBACK_COLLECTION).updateOne({user_data:objectId(userId)},{
                    $push:{
                        feedbacks:feedback
                    }
                }).then(async()=>{
                    await db.get().collection(collection.ORDER_COLLECTION).updateMany({user_data:objectId(userId),delivery:'pending'},{
                        $set:{
                            delivery:'delivered',
                            status:'Delivered'
                        }
                ...
                await db.get().collection(collection.FEEDBACK_COLLECTION).insertOne({
                    user_data:objectId(userId),
                    feedbacks:[feedback]
                }).then(async()=>{
                    await db.get().collection(collection.ORDER_COLLECTION).updateOne({user_data:objectId(userId),delivery:'pending'},{
                        $set:{
                            delivery:'delivered',
                            status:'Delivered'
                        }
    ...
    getFeedback:()=>{
        return new Promise(async (resolve,reject)=>{
            await db.get().collection(collection.FEEDBACK_COLLECTION).aggregate([
                {
                    $match:{
    ...
    getMonthlyData:()=>{
        return new Promise(async (resolve,reject)=>{
            ...
            console.log(thisYear);
            let monthData=await db.get().collection(collection.ORDER_COLLECTION).aggregate([
                {
                    $project:{
                        month: { $month: "$date" },
                        total_amount:1,
    ...
    getMessage:()=>{
        return new Promise(async (resolve,reject)=>{
            await db.get().collection(collection.MESSAGE_COLLECTION).aggregate([
                {
                    $match:{
    ...
    //     if(startPrice>=200){
    //         return new Promise(async (resolve,reject)=>{
    //             let products=await db.get().collection(collection.PRODUCT_COLLECTION).find({price:{$gt:startPrice}}).toArray()
    ...
    //                 $and:[{price:{$gt:startPrice}},{price:{$lt:endPrice}}]
    //             }).toArray()
    //             if(products.length>0){
    //                 console.log(products);
    //                 resolve(products)
